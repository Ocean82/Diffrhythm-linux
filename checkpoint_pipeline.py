#!/usr/bin/env python3
"""
Checkpoint-based pipeline that saves intermediate results to disk
"""
import os
import sys
import time
...
class CheckpointPipeline:
    ...
    def step2_style(self, style_prompt, force_rerun=False):
        ...
        muq = MuQMuLan.from_pretrained(
            "OpenMuQ/MuQ-MuLan-large", cache_dir="./pretrained"
        )
        ...
        # Save checkpoint
        checkpoint_data = {"style": style, "negative_style": negative_style}
    ...
    def step3_generation(self, lyrics_data, style_data, steps=16, force_rerun=False):
        ...
        print(
            f"  ✓ Generation completed in {gen_time:.1f}s ({gen_time/60:.1f} minutes)"
        )

        # Save checkpoint
        checkpoint_data = {"latents": latents, "generation_time": gen_time}
    ...
    def step4_decode(
        self,
        generation_data,
        output_path="./output/checkpoint_output.wav",
        force_rerun=False,
    ):
    ...
    def run_pipeline(
        ...
        """Run the complete checkpoint pipeline"""
        print("DiffRhythm Checkpoint Pipeline")
        print("=" * 40)
        print("This approach saves progress at each step and can resume from failures")
        print("Perfect for unreliable systems or long-running processes")
        print("")

        if resume:
            print("Resume mode: Will skip completed steps")
        else:
            print("Fresh run: Will recompute all steps")

        total_start = time.time()

        try:
            # Step 1: Lyrics
            lyrics_data = self.step1_lyrics(lyrics_text, force_rerun=not resume)

            # Step 2: Style
            style_data = self.step2_style(style_prompt, force_rerun=not resume)

            # Step 3: Generation (the slow part)
            generation_data = self.step3_generation(
                lyrics_data, style_data, steps=16, force_rerun=not resume
            )

            # Step 4: Decode
            success, final_path = self.step4_decode(
                generation_data, output_path, force_rerun=not resume
            )
            ...
            print("")
            print("Benefits of checkpoint approach:")
            print("- Can resume from any step if interrupted")
            print("- Minimal memory usage (one model at a time)")
            print("- Progress is never lost")
            print("- Can rerun individual steps if needed")

            return success

        except Exception as e:
            print(f"\n✗ Pipeline failed: {e}")
            print("Checkpoints are saved - you can resume from where it failed")
            import traceback
...
def main():
    ...
    print("DiffRhythm Checkpoint Pipeline")
    print("=" * 35)
    print("Saves progress at each step - can resume from failures")
    print("")
...
[00:20.00]Perfect for CPU systems"""
    ...
    choice = input("Run checkpoint pipeline? (y/n): ").lower().strip()
    if choice != "y":
        print("Cancelled.")
        return

    resume = input("Resume from existing checkpoints? (y/n): ").lower().strip() == "y"

    # Create pipeline
    pipeline = CheckpointPipeline()

    # Run pipeline
    success = pipeline.run_pipeline(lyrics, style, resume=resume)

    if success:
        print("\n🎵 SUCCESS! Check ./output/checkpoint_output.wav")
        print("Checkpoints saved in ./output/checkpoints/")
    else:
        print("\n⚠ Check the output and checkpoint files")
