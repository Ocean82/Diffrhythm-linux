...
try:
    ...
    # These are optional dependencies for Chinese text processing
    jieba = None
    lazy_pinyin = None
    Style = None
...
def mask_from_start_end_indices(
    seq_len: int["b"], start: int["b"], end: int["b"], max_frames
):  # noqa: F722 F821
...
def mask_from_frac_lengths(
    seq_len: int["b"], frac_lengths: float["b"], max_frames
):  # noqa: F722 F821
...
def convert_char_to_pinyin(text_list, polyphone=True):
    if jieba is None or lazy_pinyin is None or Style is None:
        raise ImportError(
            "jieba and pypinyin are required for Chinese text processing. "
            "Install them with: pip install jieba pypinyin"
        )
